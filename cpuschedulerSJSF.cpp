    ...
    while(!readyQueue.empty()){
        ...
        for(int i=0;i<processes.size();i++){
            if(ioreturn[i].first <= time){
                readyQueue.push(make_pair(processes[ioreturn[i].second][processBurstIndex[ioreturn[i].second]], ioreturn[i].second)); //pushing process id into readyQueue
...
//printing results
cout<<"Writen by EMIN MUGLA (Z23371337)"<<endl;
cout<<"******************************************************** "<<endl;
