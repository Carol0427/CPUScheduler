...
void fcfs(vector<vector<int>> processes, queue<int> readyQueue, vector<int> processBurstIndex);
...
cout<<endl<<"----FCFS----"<<endl;
 fcfs(processes,readyQueueFCFS,processBurstIndex);
 }
...
int idle_Cpu_Time=0;
 ...
 int process_index = readyQueue.front();
 readyQueue.pop();
 ...
 readyQueue.push(ioreturn[i].second); //pushing process id into readyQueue
...
ioreturn[i].first = INT_MAX;
 ...
 while (!q.empty()){
 int pid = q.front();
 cout<<setfill(' ')<<setw(10)<<"P"<<pid<<setw(10)<<processes[pid][processBurstIndex[pid]]<<endl;
 q.pop();
 }
 }else{
